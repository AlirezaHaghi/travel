...
class StrategyAgent:
            ...
            for i in range(max_try):
                ...
                if result and 'answer' in result:
                    ...
                    if match:
                        ...
                        try:
                            ...
                            if not isinstance(daily_plan_raw, dict) or not all(
                                isinstance(k, str)
                                and k.startswith("day")
                ...
                for selected_spot_info in selected_data:
                    if selected_spot_info["name"] not in current_plan_attraction_names:
                        print(
                            f"Validation Failed: Selected spot '{selected_spot_info['name']}' not in the generated plan {current_plan_attraction_names}."
                        )
                        valid_plan = False
                        break
    ...
    def extract_rental_recommendation(self, recommendation_text):
        ...
        try:
            ...
            car_rental_section_match = re.search(
                r'car rental recommendation:(.+?)(?=\n\n|\Z)', recommendation_text, re.DOTALL | re.IGNORECASE
            )
            if car_rental_section_match:
                ...
                if (
                    car_rental_section.startswith("no")
                    or "i do not recommend" in car_rental_section
                    or "i don't recommend" in car_rental_section
                ):
            ...
            if (
                "not recommend renting a car" in recommendation_text
                or "do not recommend renting a car" in recommendation_text
            ):
            ...
            print(
                f"[CRITICAL] Could not determine car rental recommendation from text. Full original text: {original_text}"
            )
            print("[DEBUG] Defaulting to FALSE for safety")
            return False

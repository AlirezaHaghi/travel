document.addEventListener("DOMContentLoaded", function () {
  ...
  const paginatedRecommendationsContainer = document.getElementById(
    "paginated-recommendations-container"
  );
  const attractionContentArea = document.getElementById(
    "attraction-content-area"
  );
  const recommendationsPaginationControls = document.getElementById(
  ...
  const confirmAllSelectionsFooter = document.getElementById(
    "confirm-all-selections-footer"
  );
  const confirmSelectedAttractionsBtn = document.getElementById(
    "confirm-selected-attractions-btn"
  );
  const loadingSpinner = document.getElementById("loading-spinner");
  const resetBtn = document.getElementById("reset-btn");
  const stepNav = document.getElementById("step-nav");
  const missingFieldsContainer = document.getElementById("missing-fields");
  const selectedAttractionsList = document.getElementById(
  ...
  function showMissingFields(fields) {
    ...
    missingFieldsContainer.innerHTML =
      ...
      fields
        .map((f) => `<span class="badge bg-warning text-dark me-1">${f}</span>`)
        .join("");
  ...
  function processUserInput(message) {
    ...
    if (state.step === "recommend" && state.selectedAttractions.length > 0) {
      params.append(
        ...
        JSON.stringify(state.selectedAttractions.map((a) => a.id))
      );
    }
    ...
    if (state.ai_recommendation_generated !== undefined) {
      params.append(
        ...
        state.ai_recommendation_generated.toString()
      );
    }
    if (state.user_input_processed !== undefined) {
      params.append(
        "user_input_processed",
        state.user_input_processed.toString()
      );
    }
    ...
    eventSource.onmessage = function (event) {
      ...
      if (data.type === "chunk") {
        ...
        console.log(
          "[DEBUG] processUserInput - Received COMPLETE message with data:",
          JSON.parse(JSON.stringify(data))
        );

        if (data.attractions) {
          console.log(
            "[DEBUG] processUserInput - data.attractions received:",
            JSON.parse(JSON.stringify(data.attractions))
          );
        }
        if (data.map_data) {
          console.log(
            "[DEBUG] processUserInput - data.map_data received:",
            JSON.parse(JSON.stringify(data.map_data))
          );
        }
        ...
        if (data.state) {
          ...
          if (data.state.attractions)
            state.attractions = data.state.attractions;
          if (data.state.selected_attractions)
            state.selectedAttractions = data.state.selected_attractions;
          ...
          if (data.state.ai_recommendation_generated !== undefined) {
            state.ai_recommendation_generated = Boolean(
              data.state.ai_recommendation_generated
            );
            console.log(
              "[DEBUG] Updated ai_recommendation_generated:",
              state.ai_recommendation_generated
            );
          }
          if (data.state.user_input_processed !== undefined) {
            state.user_input_processed = Boolean(
              data.state.user_input_processed
            );
            console.log(
              "[DEBUG] Updated user_input_processed:",
              state.user_input_processed
            );
          }
        }
        ...
        if (state.step === "complete") {
          ...
          addChatMessage(
            "Your itinerary and budget have been generated! Check the left panel for details.",
            "assistant"
          );
        }
        ...
        if (state.step === "strategy" && data.next_step === "strategy") {
          ...
          if (userInput) {
            userInput.value =
              "I am satisfied with your recommendation, let us go to next step";
            userInput.focus();
          }
        }
    ...
    data.forEach((attraction) => {
      ...
      const marker = L.marker([
        ...
        attraction.location.lng,
                    ...
                    <button onclick="selectAttraction('${
                      attraction.id || ""
                    }')">Select</button>
                `);
  ...
  function updateAttractions(attractions) {
    console.log(
      "[DEBUG] updateAttractions - Top level (raw) attractions argument received:",
      attractions
    );

    if (attractions && attractions.length > 0) {
      console.log(
        "[DEBUG] updateAttractions - First attraction object (raw):",
        attractions[0]
      );
      if (attractions[0]) {
        console.log(
          "[DEBUG] updateAttractions - First attraction's location object (raw):",
          attractions[0].location
        );
      }
      if (attractions.length > 1 && attractions[1]) {
        console.log(
          "[DEBUG] updateAttractions - Second attraction object (raw):",
          attractions[1]
        );
        if (attractions[1]) {
          console.log(
            "[DEBUG] updateAttractions - Second attraction's location object (raw):",
            attractions[1].location
          );
        }
      }
    }
    ...
    if (attractions && attractions.length > 0) {
      console.log(
        "[INFO] Performing data integrity check on received attractions..."
      );
      for (let i = 0; i < attractions.length; i++) {
        const attr = attractions[i];
        if (!attr) {
          console.error(
            `[VALIDATION_ERROR] Attraction at index ${i} is null or undefined.`
          );
          continue;
        }
        if (typeof attr.image_url !== "string" && attr.image_url !== null) {
          console.error(
            `[VALIDATION_ERROR] Attraction '${
              attr.name || "ID: " + attr.id
            }' (index ${i}) has invalid image_url:`,
            attr.image_url
          );
        }
        if (!attr.location) {
          console.error(
            `[VALIDATION_ERROR] Attraction '${
              attr.name || "ID: " + attr.id
            }' (index ${i}) is missing location object.`
          );
        } else {
          if (typeof attr.location.lat !== "number") {
            console.error(
              `[VALIDATION_ERROR] Attraction '${
                attr.name || "ID: " + attr.id
              }' (index ${i}) has invalid location.lat:`,
              attr.location.lat
            );
          }
          if (typeof attr.location.lng !== "number") {
            console.error(
              `[VALIDATION_ERROR] Attraction '${
                attr.name || "ID: " + attr.id
              }' (index ${i}) has invalid location.lng:`,
              attr.location.lng
            );
          }
        }
    ...
    try {
      console.log(
        "[DEBUG] updateAttractions - Received attractions (deep copy for log):",
        JSON.parse(JSON.stringify(attractions))
      );
    } catch (e) {
      console.warn(
        "[DEBUG] updateAttractions - Could not deep copy attractions for logging, showing raw:",
        attractions
      );
    }
    ...
    if (!currentAttractions || currentAttractions.length === 0) {
      // Changed from attractions to currentAttractions
    ...
    console.log(
      "[DEBUG] Pagination controls classes:",
      recommendationsPaginationControls.className
    );
    console.log(
      "[DEBUG] Pagination controls display style:",
      window.getComputedStyle(recommendationsPaginationControls).display
    );
  ...
  function renderCurrentAttractionPage() {
    ...
    console.log(
      `[DEBUG] renderCurrentAttractionPage - current page index: ${currentRecommendationPage}`
    );
    ...
    if (attraction) {
      console.log(
        "[DEBUG] renderCurrentAttractionPage - Attraction object from currentAttractions array:",
        JSON.parse(JSON.stringify(attraction))
      );
      console.log(
        "[DEBUG] renderCurrentAttractionPage - Attraction's location object from currentAttractions array:",
        attraction.location
          ? JSON.parse(JSON.stringify(attraction.location))
          : "LOCATION IS UNDEFINED OR NULL"
      );
    } else {
      console.error(
        `[ERROR] renderCurrentAttractionPage - No attraction found in currentAttractions at index ${currentRecommendationPage}`
      );
    }

    if (!attraction) {
      // This check is now somewhat redundant due to the logging above, but keep for safety
      console.error(
        `[ERROR] No attraction data found for page index: ${currentRecommendationPage}`
      );
      attractionContentArea.innerHTML =
        '<p class="text-center text-muted">Error displaying attraction data.</p>';
      return;
    }
    console.log(
      "[DEBUG] Rendering attraction:",
      JSON.parse(JSON.stringify(attraction))
    ...
    let rating = attraction.rating
      ? `⭐ ${attraction.rating} (${
          attraction.user_ratings_total || 0
        } reviews)`
      : "No rating";
    let duration = attraction.estimated_duration
      ? `${attraction.estimated_duration} hours (est.)`
      : "Duration not specified";

    const isSelected = selectedAttractions.some(
      (sa) => sa.id === attraction.id
    );
    ...
    attractionPageDiv.innerHTML = `
      ...
      attraction.name || "Attraction"
    }" class="img-fluid rounded mb-2 attraction-image" style="max-height: 200px; width: 100%; object-fit: cover;">
                    <h5 class="attraction-name mb-1">${
                ...
                <div class="col-md-7">
                    ...
                    } select-attraction-btn" data-attraction-id="${
      attraction.id
    }">
                        <i class="fas ${
                          isSelected ? "fa-check-circle" : "fa-plus-circle"
                        }"></i> ${
      isSelected ? "Selected" : "Select this Attraction"
    }
                    </button>
                </div>
            </div>
            <hr>
            <div class="row">
                 <div class="col-12 nearby-info-slot" id="nearby-info-${
                   attraction.id
                 }">
  ...
  function toggleAttractionSelection(attraction, button) {
    ...
    console.log(
      "[DEBUG] typeof removeMarkerFromMap:",
      typeof removeMarkerFromMap
    );
    ...
    if (index > -1) {
      ...
      console.log(
        "[DEBUG] Select button DESELECTED. Classes:",
        button.className
      );
    } else {
  ...
  function updatePaginationInfo() {
    attractionPageInfo.textContent = `Attraction ${
      currentRecommendationPage + 1
    } of ${currentAttractions.length}`;
    prevAttractionBtn.disabled = currentRecommendationPage === 0;
    nextAttractionBtn.disabled =
      currentRecommendationPage === currentAttractions.length - 1;
  }
  ...
  confirmSelectedAttractionsBtn.addEventListener("click", () => {
    if (selectedAttractions.length > 0) {
      ...
      addChatMessage(
        "Please select at least one attraction from the recommendations.",
        "assistant"
      );
    }
  });
  ...
  function addMarkerToMap(attraction) {
    if (
      ...
      console.error(
        "[ERROR] addMarkerToMap: Invalid attraction data or map not initialized.",
        attraction
      );
      return;
    }
    ...
    const existingMarkerIndex = mapMarkers.findIndex(
      (m) => m.attractionId === attraction.id
    );
  ...
  function removeMarkerFromMap(attractionId) {
    const markerIndex = mapMarkers.findIndex(
      (m) => m.attractionId === attractionId
    );
    if (markerIndex !== -1) {
  ...
  function updateItinerary(itinerary) {
    ...
    if (!itinerary || itinerary.length === 0) {
      itineraryContainer.innerHTML =
        '<p class="text-center text-muted">No itinerary available yet.</p>';
      return;
    }
  ...
  function updateBudget(budget) {
    ...
    budgetDiv.innerHTML = `
        ...
        <div class="row">
            <div class="col-md-6">
                <p class="mb-1"><strong>Total:</strong> $${Number(
                  budget.total
                ).toFixed(2)}</p>
                <p class="mb-1"><strong>Accommodation:</strong> $${Number(
                  budget.accommodation
                ).toFixed(2)}</p>
                <p class="mb-1"><strong>Food:</strong> $${Number(
                  budget.food
                ).toFixed(2)}</p>
            </div>
            <div class="col-md-6">
                <p class="mb-1"><strong>Transport:</strong> $${Number(
                  budget.transport
                ).toFixed(2)}</p>
                <p class="mb-1"><strong>Attractions:</strong> $${Number(
                  budget.attractions
                ).toFixed(2)}</p>
                ${
                  budget.car_rental
                    ? `<p class="mb-1"><strong>Car Rental:</strong> $${Number(
                        budget.car_rental
                      ).toFixed(2)}</p>`
                    : ""
                }
                ${
                  budget.fuel_cost
                    ? `<p class="mb-1"><strong>Fuel Cost:</strong> $${Number(
                        budget.fuel_cost
                      ).toFixed(2)}</p>`
                    : ""
                }
  ...
  function updateConfirmation(response) {
    ...
    // 支持 markdown 格式
    confirmationDiv.innerHTML = `<div class="message-content">${marked.parse(
      response
    )}</div>`;
  }
    ...
    fetch("/api/reset")
      .then(() => {
        ...
        recommendationsContainer.innerHTML =
          '<p class="text-center text-muted">Recommendations will appear here based on your preferences.</p>';
...
`,
    ...
    attractions.forEach((attraction) => {
      ...
      card.innerHTML = `
                <div class="card-body">
                    ...
                    <small class="text-muted">${
                      attraction.estimated_duration || 2
                    } hours</small>
                </div>
            `;
  ...
  function removeAttraction(attractionId) {
    ...
    selectedAttractions = selectedAttractions.filter(
      (a) => a.id !== attractionId
    );
    updateSelectedAttractionsList();
    ...
    selectedMarkersLayer.eachLayer((layer) => {
      if (
        layer
          .getLatLng()
          .equals([attraction.location.lat, attraction.location.lng])
      ) {
  ...
  function drawRoute(route) {
    ...
    if (Object.keys(allPolylinesGroup.getLayers()).length > 0) {
      ...
      const singleMarkersGroup = L.featureGroup(
        route.map((spot) => L.marker([spot.location.lat, spot.location.lng]))
      );
  ...
  function fetchNearbyPlaces(attraction, containerId) {
    ...
    nearbyContainer.innerHTML =
      '<p class="text-muted">Loading nearby information...</p>'; // Ensure this is set before fetch

    console.log(
      "[DEBUG] fetchNearbyPlaces - Attraction:",
      JSON.parse(JSON.stringify(attraction)),
      "Container ID:",
      containerId
    );
    if (
      ...
      console.error(
        "[ERROR] Invalid attraction object or location for nearby search. Attraction object:",
        attraction
      );
      if (attraction) {
        // Log location details if attraction object itself exists
        console.error(
          "[ERROR] Attraction location object:",
          attraction.location
        );
        if (attraction.location) {
          console.error(
            `[ERROR] Attraction location.lat: ${
              attraction.location.lat
            } (type: ${typeof attraction.location.lat})`
          );
          console.error(
            `[ERROR] Attraction location.lng: ${
              attraction.location.lng
            } (type: ${typeof attraction.location.lng})`
          );
        } else {
          console.error(
            "[ERROR] attraction.location itself is undefined or null."
          );
        }
      } else {
      ...
      if (nearbyContainer)
        nearbyContainer.innerHTML =
          '<p class="text-danger">Error: Invalid attraction data for nearby search.</p>';
      return;
    }
    ...
    console.log(
      `[DEBUG] Fetching nearby for ${attraction.name} with coordinates: ${coordinates}`
    );
    ...
    fetch(`/api/nearby/${coordinates}`)
      ...
      .then((data) => {
        console.log(
          `[DEBUG] Nearby data received for ${attraction.name}:`,
          JSON.parse(JSON.stringify(data))
        );
      ...
      .catch((error) => {
        console.error(
          `[ERROR] Error fetching nearby places for ${attraction.name}:`,
          error
        );
        nearbyContainer.innerHTML =
          '<p class="text-danger">Could not load nearby information. Check console for details.</p>';
      });
  }
  ...
  function formatNearbyPlacesMessage(data, attractionName) {
    ...
    if (data.restaurants && data.restaurants.length > 0) {
      ...
      data.restaurants.forEach((restaurant) => {
        ...
        message += `<strong>${restaurant.name}</strong> (${
          restaurant.type || "Restaurant"
        }) - Rating: ${restaurant.rating || "N/A"}⭐, Price: ${
          "💰".repeat(restaurant.price_level || 0) || "N/A"
        } <br><small>${restaurant.address || ""}</small>`;
        message += `</li>`;
      });
      message += "</ul>";
    } else {
